...
namespace SkyLinesLibrary
{
    ...
    public class FlightDL_DB :FlightsDL,IFlightDL
    {
        ...
        {
            string query = string.Format("INSERT INTO Flights(FlightID,FlightName,Source,Destination,TravelDate,TakeoffTime,Price,Seats,Discount)" + "Values ('{0}','{1}','{2}','{3}','{4}','{5}','{6}','{7}','{8}')", fl.GetFlightID(), fl.GetFlightName(), fl.GetSource(), fl.GetDestination(), fl.GetTravelDate(), fl.GetTakeoffTime(), fl.GetPrice(), fl.GetSeats());
            SqlCommand cmd = new SqlCommand(query, db.GetConnection());
            cmd.ExecuteNonQuery();
        }
